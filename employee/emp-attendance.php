...
if (isset($_POST['submitTimeIn'])) {
    ...
    try {
        ...
        if ($count > 0) {
            ...
            // Check if an image file was uploaded and it's of allowed types (PNG, JPG, JPEG)
...
if (isset($_POST['submitTimeOut'])) {
    ...
    try {
        ...
        // Calculate getsalary based on num_hr
        $getsalary = $totalHours * 73;

        // Update the 'num_hr' and 'getsalary' columns in the tblattendance table using prepared statement
        $sqlUpdateNumHrs = "UPDATE tblattendance SET num_hr = :numHrs, getsalary = :getsalary WHERE emp_names = :employee_name AND time_in_datetime = :time_in_datetime";
        $queryUpdateNumHrs = $dbh->prepare($sqlUpdateNumHrs);
        $queryUpdateNumHrs->bindParam(':numHrs', $totalHours, PDO::PARAM_STR);
        $queryUpdateNumHrs->bindParam(':getsalary', $getsalary, PDO::PARAM_STR);
...
<form method="post" enctype="multipart/form-data">
  ...
  <div class="form-group">
    ...
    <input type="file" name="selfie" class="form-control" accept="image/png, image/jpeg, image/jpg" required>
  </div>
